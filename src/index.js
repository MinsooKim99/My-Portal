...
const TEXT_MODEL = "@cf/meta/llama-3.1-8b-instruct"; // 텍스트 대화/답장 (빠르고 무료 소모 적음)
...
async function handleChat(request, env) {
  ...
  const r = await env.AI.run(TEXT_MODEL, { messages: withSystem, max_tokens: 1024 });
  return json({ answer: r.response || "" });
}
...
async function processDiscordCommand(name, opts, interaction, env) {
  ...
  try {
    if (name === "chat") {
      ...
      await patchFollowup(followupUrl, { content: (r.response || "(빈 응답)").slice(0, 1900) });
    } else if (name === "image") {
      ...
      await patchFollowup(followupUrl, { content: (r.response || "").slice(0, 1900) });
    }
  } catch (err) {
